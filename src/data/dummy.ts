...
export type SalonProfile = {
  ...
  isOpen: boolean;
  description: string;
  barbers: { id: string; name: string; initials: string }[];
};

export const dummySalonProfiles: SalonProfile[] = [
  {
    id: 'p1',
    name: 'The Barber Shop',
    rating: 4.9, reviews: 312, distanceKm: 0.7, priceFrom: 35, isOpen: true,
    description: 'Premier destination for precision cuts and expert grooming in the heart of the city. Our master barbers combine classic techniques with modern style.',
    barbers: [
      { id: 'b1', name: 'Richard Anderson', initials: 'RA' },
      { id: 'b2', name: 'Marcus Bell',      initials: 'MB' },
      { id: 'b3', name: 'Dawit Alem',       initials: 'DA' },
      { id: 'b4', name: 'Theo Brooks',      initials: 'TB' },
      { id: 'b5', name: 'Iván Costa',       initials: 'IC' },
    ],
  },
  {
    id: 'p2',
    name: 'Good Place',
    rating: 4.8, reviews: 198, distanceKm: 2.3, priceFrom: 30, isOpen: true,
    description: 'A welcoming neighbourhood barbershop known for clean fades and personalised service. Walk-ins welcome, appointments preferred.',
    barbers: [
      { id: 'b1', name: 'Richard Anderson', initials: 'RA' },
      { id: 'b2', name: 'Marcus Bell',      initials: 'MB' },
      { id: 'b3', name: 'Dawit Alem',       initials: 'DA' },
  ...
  },
  {
    id: 'p3',
    name: 'Silent Fox',
    rating: 5.0, reviews: 127, distanceKm: 1.2, priceFrom: 50, isOpen: false,
    description: 'Boutique barbershop specialising in high-end grooming experiences. By appointment only.',
    barbers: [
      { id: 'b4', name: 'Theo Brooks', initials: 'TB' },
      { id: 'b5', name: 'Iván Costa',  initials: 'IC' },
    ],
  ...
  {
    id: 'p4',
    name: 'Rogers Barbershop',
    rating: 4.7, reviews: 279, distanceKm: 3.1, priceFrom: 35, isOpen: true,
    description: 'Classic barbershop experience with a modern twist. Known for exceptional fades and beard work across multiple locations.',
    barbers: [
      { id: 'b1', name: 'Richard Anderson', initials: 'RA' },
      { id: 'b2', name: 'Marcus Bell',      initials: 'MB' },
      { id: 'b3', name: 'Dawit Alem',       initials: 'DA' },
      { id: 'b4', name: 'Theo Brooks',      initials: 'TB' },
      { id: 'b5', name: 'Iván Costa',       initials: 'IC' },
      { id: 'b6', name: 'Jordan Price',     initials: 'JP' },
    ],
  },
  {
    id: 'n1',
...
export const dummyBarberProfiles: BarberProfile[] = [
  { id: 'b1', name: 'Richard Anderson', initials: 'RA', isPro: true,  rating: 4.9, reviews: 114, defaultServiceDurationMin: 45 },
  { id: 'b2', name: 'Marcus Bell',      initials: 'MB', isPro: false, rating: 4.7, reviews: 88,  defaultServiceDurationMin: 30 },
  { id: 'b3', name: 'Dawit Alem',       initials: 'DA', isPro: true,  rating: 4.8, reviews: 67,  defaultServiceDurationMin: 45 },
  { id: 'b4', name: 'Theo Brooks',      initials: 'TB', isPro: false, rating: 4.6, reviews: 54,  defaultServiceDurationMin: 30 },
  { id: 'b5', name: 'Iván Costa',       initials: 'IC', isPro: false, rating: 4.8, reviews: 92,  defaultServiceDurationMin: 30 },
  { id: 'b6', name: 'Jordan Price',     initials: 'JP', isPro: false, rating: 4.5, reviews: 41,  defaultServiceDurationMin: 30 },
];

export type Availability = Record<string, { time: string; available: boolean }[]>;

export function getMockAvailability(barberId: string, year: number, month: number): Availability {
  const daysInMonth = new Date(year, month, 0).getDate();
  const baseSlots = ['09:00','09:30','10:00','10:30','11:00','11:30','12:00','14:00','14:30','15:00','15:30','16:00','16:30','17:00'];
  const seed = barberId.charCodeAt(barberId.length - 1);
  const result: Availability = {};
  for (let d = 1; d <= daysInMonth; d++) {
    if (new Date(year, month - 1, d).getDay() === 0) continue; // closed Sunday
    const dateStr = `${year}-${String(month).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
    result[dateStr] = baseSlots.map((time, i) => ({
      time,
      available: (seed + d + i) % 3 !== 0,
    }));
  }
  return result;
}
